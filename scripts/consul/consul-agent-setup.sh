...
if [ -x "$(which wget unzip)" ] ; then
  echo "dependencies found moving to next step..."
else
...
NODE_ADDR=$(cat /tmp/consul-node-addr | tr -d '\n')

# Write the flags to a temporary file
cat >/tmp/consul_flags << EOF
CONSUL_FLAGS="-data-dir=/opt/consul/data -node=${NODE_NAME} -bind=${NODE_ADDR} -enable-script-checks=true -config-dir=/etc/consul.d"
EOF
